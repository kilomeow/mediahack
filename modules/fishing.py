...
content = StoryMap(
    entry = [
        ...
        npc.Owl.ask("Давайте-ка я подключу вас к серверу моих знакомых ящериц из Калифорнии!", [("Включить VPN", "ready")], Var.null),
        # connect
    ...
    Я очень рад, что наконец-то смог связаться с Вами, несмотря на то, что ситуация с пожарами настолько сильно обострилась.

    Весной этого года я руководил вырубкой лесного массива в Грибном районе, поэтому часть ответсвенности за то, что происходит сейчас, лежит и на мне. Моя бригада действовала согласно приказу начальника пожарной службы района, однако сектор, который был выделен для нас, был населён крепкими, взрослыми деревьями. 
    Наверное, Вы и так уже знаете, но я повторю: в таких ситуациях необходимо вырубать исключительно сухие деревья. Обычно перед подобными мероприятиями проводится экспертиза каждого сектора, поэтому мы тут же связались с начальством и потребовали дополнительную проверку. 

    В итоге мы получили отказ в устной форме, и теперь у меня нет никаких свидетельств того, что я действительно пытался помешать уничтожению здоровых деревьев. 

    У меня есть все основания полагать, что в приказе не было никакой ошибки. Дело в том, что все вырубленные в ходе противопожарных мер деревья в конечном итоге отправляются на целлюлозный завод, однако крупные, здоровые деревья по нашим лесным законам не подходят для изготовления бумаги. Если вам удастся узнать, куда были отправлены эти деревья, то всё встанет на свои места.
    ...
    letter = [
        ...
        #В СТРОКЕ НИЖЕ ДОЛЖНА БЫТЬ КНОПКА
        npc.Magpie.info("Зарегистрироваться и ознакомиться со списком запланированных мероприятий можно по ссылке: leznews.conf.orc"),
    ...
    ending1 = [
        ...
        npc.Magpie.info("""
    Привет всем!
    Помните, Сорокин хвост опубликовал заявление какого-то парня из пожарной службы? Нам это показалось очень важным, поэтому мы решили покружить над рекой Камышовкой и сравнить её фотографии с тем, что было в прошлом году.
    В дельте реки мы обнаружили солидную плотину, сделанную из крепких деревьев. Спилы на этих деревьях очень ровные, это не похоже на самострой. В прошлом году в этом же месте почти ничего не было, там торчало всего несколько палок, но теперь здесь просто гигантская дамба. 

    Всё бы ничего, если бы рядом с Камышовкой не жил начальник пожарной службы бобёр Арсений. Кажется, скоро у нас с Сорокиным хвостом будет очень крутое расследование про коррупцию в пожарной службе!

    Ждите новый мега триллер от дроздов!"""),
        npc.Floppa.say("ВАУ"),
        npc.Floppa.say("Серьёзно???"),
        npc.Squirrel.ask("Нам нужно помочь дроздам! Надо поделиться их заявлением!", [("Опубликовать сообщение", "ready")], Var.null),
        #В СТРОКЕ НИЖЕ ПОЯВЛЯЕТСЯ ШКАЛА!
        score.add(2),
        ...
        npc.Magpie.info("""
        Мы всегда внимательно относились к нашим журналистам, и последние дни не исключение. Однако мы не хотим, чтобы важные для нашего сообщества вопросы оставались освещёнными только в прессе, поэтому мы начинаем проверку по факту незаконного строительства плотины на реке Камышовке.
