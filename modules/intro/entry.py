...
content = StoryMap(
    entry = [
        ...
        #npc.Squirrel.say("Пожалуйста, напишите, как к вам обращаться. Например, я - игуана"),
        ...
        npc.Squirrel.info("Как вы знаете, Сорокин хвост существует для укрепления горизонтальных связей внутри сообщества птиц, зверей и рыб в Хвойном Лесу и соседних районах. Мы стараемся публиковать не только свои тексты, но и материалы читателей, чтобы делать наш общий дом каждый день немного лучше"),
        npc.Floppa.say("Недавно нам прислали рецепты очень вкусных пельменей..."),
        npc.Squirrel.say("Это лето обещает быть крайне тяжёлым для нашего маленького коллектива, поэтому нам необходимы новые заряженные этнузиазмом сотрудники. Я очень рада, что вы решили с нами поработать!"),
        ...
        npc.Squirrel.say("Сорока - наша коррепсондентка. Из-за нехватки кадров не ней сейчас висят ещё и читательские материалы, но она иногда не успевает делать всё сразу("),
        npc.Squirrel.say("Ребята, вы прочитали этот текст?"),
        npc.Squirrel.say("Его нужно отредактировать, но тут есть одна проблемка. В нём могут содержаться личные данные."),
        ...
        npc.Owl.say("Итак. Во время работы вы будете постоянно сталкиваться с различными трудностями. И анонимизация ваших респондентов одна из таких проблем."),
        npc.Owl.info("По закону о СМИ вы не имеете права разглашать чужие данные, будь то имя или место жительства, без разрешения. Иногда нас действительно просят не указывать имена и прочее, но в других случаях решение остаётся за вами. Сорока же имена не успевает убирать, поэтому вам придётся взять это на себя, чтобы никто не пострадал, в том числе и мы с вами."),
        ...
        npc.Floppa.info("Сроки у нас не особо горящие, но вам нужно научиться быстро договариваться друг с другом, это суперрррр важно"),
        ...
        score.instantiate(),
        score.add(2),
        npc.Floppa.say("Это шкала вашей крутости..... Чем вы (мы) круче, тем быстрее распространяются наши материлы в лесу! А чем больше у нас читателей, тем мы круче! А чем мы круче, тем больше у нас читателей!"),
        ...
        npc.Squirrel.say("Давайте сделаем так. Я сейчас вышлю темы, а наши новенькие выберут, с чего им будет лучше начинать!"),
        npc.Squirrel.ask("Вот примерный список! Нажимайте на кнопку, и мы немедленно приступим к работе!", [("Социальные сети, мессенджеры, пароли", "social_media")], Var.module),
        Proceed('social')
    ],

    social = [
        npc.Squirrel.say("Сорока, как у вас?"),
        npc.Magpie.say("почти закончили"),
        npc.Magpie.say("добавь кроля в чат я зашиваюсь"),
        npc.Squirrel.invite(npc.Rabbit),
        npc.Squirrel.say("Коллеги, сейчас давайте внимательно, времени мало"),
        npc.Squirrel.say("Кролик сотрудник Нашего дома, они с Сорокой кое-что подготовили. Но некоторые моменты ещё нужно доредактировать"),
        npc.Squirrel.say("Займитесь этим и присылайте мне, ок?"),
        npc.Floppa.say("Ещё один партнер??? Лаааадно...))"),
        npc.Floppa.say("А что за наш дом-то? "),
        npc.Rabbit.say("мы сделали лагерь для погорельцев из грибного района"),
        npc.Rabbit.say("столько там зверей и птиц, вы бы видели"),
        npc.Rabbit.say("грустно смотреть"),
        npc.Floppa.say("И они хотят жить в Хвойном лесу?"),
        npc.Rabbit.say("ну а где? "),
        npc.Floppa.say("Кажется, тут им не все будут рады("),
        npc.Rabbit.say("вот за этим мы к вам и пришли"),
        npc.Rabbit.say("нужно помочь им нормально обустроиться"),
        ...
        npc.Squirrel.ask(docs['note1'], [("Готово", "ready")], Var.null),
        score.add(3),

        npc.Magpie.say("смотрите что нам пишут на горячую линию:"),
...
Прекращайте этот балаган немедленно. """),
        ...
        score.add(3),
        npc.Magpie.say("очередная угроза .........."),
...
Мы настоятельно рекомендуем всем, кто хочет открыть свою пасть, немедленно её заткнуть ради своей же безопасности."""),
        ...
        npc.Owl.info("Это обычная ситуация, Кролику не стоило выкладывать эту фотографию в общий доступ. Сейчас обнаружить то или иное место по окружению достаточно легко: это может быть и вид из окна, и какое-то строение на фоне, что угодно."),
        npc.Owl.info("Кролик, бери лапы в лапы и беги к нотариусу. Тебе нужен *«Акт осмотра страницы в сети «Интернет»* двух этих скриншотов с угрозами*.* Сделаешь его, потом бегом в полицию, писать *заявление о преступлении*. Только сделать всё надо быстро. Полиция почти не занимается подобными делами, но здесь на лицо угроза физической расправы. Жалобу в телеграм я уже отправил, хотя это может ни на что не повлиять."),
        npc.Rabbit.say("мне правда нужно общаться с полицией???"),
        npc.Owl.say("ДА"),
        npc.Squirrel.say("Да!"),
        ...
        Jump(Var.threat_public, {"public": "public", "not_public": "not_public"})
    ],
    ...
    vzlom = [
        ...
        npc.Squirrel.ask("Подумайте хорошенько перед тем как принимать решение! У вас только одно нажатие на кнопку, обсудите ваши идеи в чате.", [("Кролик взломан", "y"), ("Кролик не взломан", "n")], Var.kick_rabbit),
        Jump(Var.kick_rabbit, {"y": "deleted", "n": "not_deleted"})
    ],

    deleted = [
        npc.Squirrel.kick(npc.Rabbit),
        npc.Squirrel.info("Надеюсь, мы поступили правильно."),
        Proceed('vzlom_advice')
    ],
