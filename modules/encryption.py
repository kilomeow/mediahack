...
content = StoryMap(
   ...
   npc.Magpie.say("Привет, это Варвара.\nНам стало известно, что кролика увезли на допрос в отдел полиции по вопросам миграции. У него отобрали телефон и от его имени написали в чат редакции Сорокиного хвоста. Мы не знаем, в каком отделении полиции он находится, и что именно ему пытаются вменять."),
   npc.Floppa.say("ЧЕГО"),
   ...
   npc.Owl.say("Первое: отключите на своих телефонах разблокировку по FaceID или по отпечатку пальца, отключите графический пароль и поставьте символьный минимум из 6 знаков. После этого установите количество попыток ввода этого самого пароля, если у вас есть такая функция."),
   npc.Magpie.say("но это же дико неудобно"),
   npc.Owl.say("Зато твой телефон будет труднее разблокировать. Бывали случаи, когда телефон отнимали, подносили к лицу и получали к нему полный доступ."),
#В СТРОКЕ НИЖЕ ДОЛЖНА БЫТЬ КНОПКА
   npc.Squirrel.say("Хорошо! Тогда давайте все поставим себе символьные пароли и установим количество попыток! [Готово]"),
#В СТРОКЕ НИЖЕ ДОЛЖНА БЫТЬ КНОПКА
   npc.Owl.say("Отлично! Вы ведь запомнили свои пароли? [Запомнили!]"),
   npc.Owl.say("Раз так, то давайте вмест сделаем одну очень важную вещь. Это поможет вам в самый критический момент."),
   npc.Owl.say("Речь идёт об автоудалении данных с ваших смартфонов. Это можно сделать, как на Android, так и на iOS. "),
   npc.Owl.say("Если у вас iOS, то вы можете настроить автоудаление данных после десяти неудачных попыток ввода пароля."),
...
#В СТРОКЕ НИЖЕ ДОЛЖНА БЫТЬ КНОПКА
   npc.Owl.say("Найти резервное копирование можно прямо в ваших настройках! Посмотрите, как оно устроено и выберите временной интервал, через который вы бы хотели сохраняться. [Понятно]"),
   npc.Magpie.say("так"),
   npc.Magpie.say("а если у меня андроид?? тогда мои данные могут забрать если они сами не удалятся???"),
#В СТРОКЕ НИЖЕ ДОЛЖНА БЫТЬ КНОПКА
   npc.Owl.say("Тут на помощь приходит шифрование данных! Оно есть и на Android, и на iOS. Зайдите в раздел 'Безопасность' на вашем устройстве и найдите там функцию с шифрованием данных. К сожалению, ваши устройства будут работать чуть медленнее, но теперь ваши данные будет сложнее прочитать, если их вытащат из ваших телефонов. [Понятно]"),
...
#В СТРОКЕ НИЖЕ ДОЛЖНА БЫТЬ КНОПКА
   npc.Owl.say("Теперь вам нужно найти в настройках вашего устройства отключение уведомлений при заблокированном экране. [Готово]"),
...
#В СТРОКЕ НИЖЕ ДОЛЖНА БЫТЬ КНОПКА
   npc.Owl.say("Тут примерно всё то же самое! Отключите биометрию, если есть, и поставьте пароли для ваших учётных записей! [Готово]"),
   npc.Owl.say("В ваших компах и ноутах могут быть встроенные сервисы для шифрования: на macOS это FireVault, на Windows это BitLocer, на Linux VeraCrypt соответственно. VeraCrypt можно использовать и на других платформах."),
#В СТРОКЕ НИЖЕ ДОЛЖНА БЫТЬ КНОПКА
   npc.Owl.say("Вот здесь есть инструкция к VeraCrypt - [https://securityinabox.org/ru/guide/veracrypt/linux/](https://securityinabox.org/ru/guide/veracrypt/linux/) [Понятно]"),
   npc.Magpie.say("а можно ли сделать что-то ещё?"),
#В СТРОКЕ НИЖЕ ДОЛЖНА БЫТЬ КНОПКА
   npc.Owl.say("Конечно! Вы дополнительно можете запаролить ваши папки, так к вашей важной информации будет ещё труднее добраться. Это особенно важно, если вы оставляете ваш ноут или комп без присмотра [Понятно]"),
...
#В СТРОКЕ НИЖЕ ДОЛЖНО БЫТЬ ГОЛОСОВАНИЕ
   npc.Squirrel.say("Давайте решать вместе, стоит ли нам как изданию поддержать Кролика ГОЛОСОВАНИЕ: Поддержать!/Не поддерживать"),
