...
content = StoryMap(
    entry=[
        npc.Squirrel.say("Привет! Наша игра познакомит вас с базовыми правилами кибербезопасности. "
                         "Сейчас я введу вас в курс дела — представлюсь сама, познакомлю вас "
        ...
        npc.Squirrel.say("Наша редакция будет признательна если вы поможете нам с несколькими текущими задачами! "
                         "В ходе работы у нас возникают проблемы в области информационной безопасности, и мы будем рады обсудить их с вами. "
                         "Мы как журналисты заботимся о сохранности данных наших информантов"),
        npc.Squirrel.say("Давайте познакомимся. Пусть каждый участник игры выберет себе эмодзи. Это важно, "
                         "чтобы я всех запомнила. Как закончите, пусть кто-то один отправит команду /done"),
        npc.Squirrel.acquaintance("Выберете себе эмодзи с животным! ", "🐶🐱🐭🐹🦊🐻🐼🐨🐯🦁🐮🐷🐸🐵🐔🐧🐺🦄"),
        #    reply_phrases=["Приятно познакомиться!", "Привееет", "Йоу! Напишите *Готово* когда закончите"]),
        npc.Squirrel.say("Ура, спасибо! Теперь я знаю всех участников"),
        npc.Squirrel.info("В игре вам предстоит коллективно принимать решения! Для этого вам нужно всем вместе нажать на одну кнопку -- например как на эту `Понятно` "),
        npc.Squirrel.info("Если кто-то из участников устанет, то имейте в виду, что из игры можно выйти используя команду `/drop` или удалившись из чата. Будьте бережны к своим силам -- дайте себе время отдохнуть если нужно. Игру можно будет продолжить позже"),
        npc.Squirrel.say("Давайте представлю вам нашу постоянную команду. "),
        ...
        npc.Squirrel.say(
            "Сорока - наша корреспондентка. Из-за нехватки кадров на ней сейчас висят ещё и читательские материалы, "
            "но она иногда не успевает делать всё сразу("),
        npc.Squirrel.say(
            "А ещё у нас есть Сова - наша безопасница. Сова, поприветствуй ребят!"),
        npc.Owl.say("Вы забыли, что я ночная птица? Я же просила вас не писать мне в это время."),
        npc.Squirrel.say(
        ...
        npc.Squirrel.say("Прямо тут, да! Чтобы посовещаться вы можете собрать общий звонок в Телеграме. Звонок можно выбрать в выпадающем меню в правом верхнем углу."),
        ...
        npc.Squirrel.info("У вас будет всего 10 минут на задание, и время пойдет после того, как вы нажмете Приступить. Будьте начеку!", button_text="Приступить"),
        npc.Squirrel.task(text=f"Вот [сообщение]({docs['fox_note']}). Итак, какие слова выдают местоположение или личные данные Кролика? Фрагментов всего 3 — напишите их "
                         "сюда, а мы уберём их из письма. Жду ваших вариантов!",
                    minutes=10,
                    fragments=['кролик Игорь', 'Пихтовой', 'Медведем'],
                    ...
                    left_fragments={3: 'В точку! Осталось ещё 3 фрагменты', 
                                    2: 'Да, да и ещё раз да! Осталось 2 фрагмента',
        ...
        npc.Squirrel.say(
            ...
            "\n"
            "Уточнение по прохождению: модуль Почта независисмый, а модули Взлом и Шифрование связаны общей сюжетной цепочкой. Начать можно с модуля про взлом."
            )
    ]
)
